...
class PgStatementsMixin(object):
    ...
    def _collect_statement_metrics(self, instance_tags):
        # Sanity checks
        if len(PG_STAT_STATEMENTS_REQUIRED_COLUMNS - self._pg_stat_statements_columns) > 0:
            self.log.warning('Unable to collect statement metrics because required fields are unavailable')
            return
        ...
        for row in rows:
            ...
            for col, (_, name, fn) in list(PG_STAT_STATEMENTS_COUNT_COLUMNS.items()) + list(PG_STAT_STATEMENTS_PER_STATEMENT_COLUMNS.items()):
                ...
                if col in PG_STAT_STATEMENTS_PER_STATEMENT_COLUMNS:
                    val = (row[col] - prev[col]) / (row['calls'] - prev['calls'])
                else:
